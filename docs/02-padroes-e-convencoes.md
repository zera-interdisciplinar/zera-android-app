...
4. **Navegação placeholder em `ManagerBottomNavBar`/`EmployeeBottomNavBar`**: todos os `ShortCutButton` navegam para `Route.Welcome` com um `TODO` indicando que as rotas reais ainda não existem (o botão central de cada uma — "Adicionar item"/"Escanear item" — também está com `onClick = {}`, sem ação real ainda).
